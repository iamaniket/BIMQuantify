...
**`ProjectPhase` enum values are language-neutral** (`design`, `tender`, `work_prep`, `shell`, `finishing`, `handover`). Display strings (Dutch terms in the wizard) live in the portal's i18n catalog and the wizard step option labels — DB values stay neutral so a German project can render German labels for the same codes. (A project no longer carries a separate `status` or `consequence_class`/Wkb-`instrument` field — `phase` is the single stage axis; see git history for the removal.)
...
**Cross-package "magic value" contracts live in `@bimdossier/contracts`**: any literal two sides of a seam must agree on — the outline/floor-plan binary magic tags (`BIMOUTL2`/`BIMFPLN2`), the WebView bridge global (`__bimdossierViewerReceive`), the federated viewer model-id format (`file-<id>`), the in-app viewer-embed asset subdir — is exported from `packages/contracts` (hand-written CommonJS, no build step, so the CommonJS Expo prebuild plugin can `require()` it and ESM/Metro/vite consumers can `import` it). Do not re-introduce a duplicated literal; import from there. The processor stays decoupled (no `@bimdossier/*` dep), so it keeps its own copies of the magic tags, pinned to the canonical values by `apps/processor/test/format-magic.test.ts` — change a format in `contracts`, the viewer codec, the processor, and that test together (and re-extract old artifacts).

**Version lockstep across packages**: `web-ifc` is pinned to `0.0.77` everywhere via root `pnpm.overrides` — bump it there (and re-verify viewer/processor/embed together), never in one package alone. `apps/viewer-embed` must keep `react` and `react-dom` exact-pinned to the **same** version (the WebView renderer needs both; a mismatch is a hard-to-trace crash, #527). The `@bimdossier/ui` custom font-size scale must stay registered in `cn.ts` (twMerge) — `scripts/check-fontsize-sync.mjs` (in `pnpm lint`) fails CI on drift; the size scale's single source is `packages/tailwind-config/index.cjs`.
