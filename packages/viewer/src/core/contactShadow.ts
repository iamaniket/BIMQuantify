...
export class ContactShadowBaker {
  ...
  bake(
    ...
    // Reframe the bake camera to the padded square footprint, looking down.
    // up = -Z makes texture +U → world +X and +V → world -Z, which lines up
    // with the display plane's UVs (PlaneGeometry rotated -PI/2 about X) so the
    // silhouette lands un-mirrored over the geometry.
