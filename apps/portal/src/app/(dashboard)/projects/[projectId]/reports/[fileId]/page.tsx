      ...
      <main className="p-6">
        <Link href={`/projects/${projectId}`}>
          <Button variant="border" size="sm">
            <ArrowLeft className="mr-1.5 h-3 w-3" /> Back to project
          </Button>
        </Link>
        <div
          role="alert"
        ...
        >
          No {framework.toUpperCase()} report found for this file. Run a check first.
        </div>
      </main>
    );
  }
        ...
        <div className="mb-3 flex items-center justify-between">
          ...
          {latestQuery.data !== undefined && (
            <span className="text-caption text-foreground-tertiary">
              {latestQuery.data.total_elements_checked} elements · {latestQuery.data.details.length} checks
            </span>
          )}
        </div>
        {latestQuery.data !== undefined ? (
