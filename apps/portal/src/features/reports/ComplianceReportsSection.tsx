...
import type { Report, ReportStatus } from '@/lib/api/schemas/reports';
...
export function ComplianceReportsSection({ projectId }: Props): JSX.Element {
  const t = useTranslations('reports.downloadable');
  const reportsQuery = useReports(projectId, 'compliance_report');
  ...
  const handleGenerate = (): void => {
    generate.mutate(
      { report_type: 'compliance_report', locale: 'nl', params: {} },
      {
        onSuccess: (report) => {
          // Auto-open the new card so the user sees the queued/running pill.
  ...
  const noComplianceData =
    generate.error instanceof ApiError &&
    generate.error.status === 422 &&
    generate.error.detail === 'NO_COMPLIANCE_DATA';
    ...
    <section className="flex flex-col gap-3 rounded-lg border border-border bg-background p-3">
      ...
      {reportsQuery.isLoading ? (
        ...
        <div className="rounded-md border border-dashed border-border px-3 py-6 text-center">
          ...
          <div className="text-body3 font-medium">{t('noReports')}</div>
        </div>
      ) : (
      ...
      <DialogContent className="max-h-[90vh] max-w-5xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            <span>{report?.title ?? t('loading')}</span>
            {report !== undefined ? (
        ...
        <DialogBody className="min-h-[60vh]">
          {isPending ? (
            ...
            <div className="text-caption text-foreground-tertiary">{t('noPreview')}</div>
          )}
        </DialogBody>
