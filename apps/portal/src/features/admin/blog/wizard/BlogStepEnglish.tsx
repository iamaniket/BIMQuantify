...
export type BlogStepEnglishProps = {
  isSubmitting: boolean;
  firstFieldRef: RefObject<HTMLInputElement | null> | undefined;
};

export function BlogStepEnglish({
  isSubmitting,
  firstFieldRef,
}: BlogStepEnglishProps): JSX.Element {
  ...
  const handleMarkdownFile = useCallback(async (file: File) => {
    ...
    // Always fill EN title + content from frontmatter / body.
    if (fm.title !== undefined) setValue('title_en', fm.title);
    setValue('content_en', fm.body);
    // Fill shared metadata only if those form fields are still empty —
    // step 1 takes precedence once the user has typed anything.
    const current = getValues();
    if (fm.description !== undefined && current.description.trim() === '') {
      setValue('description', fm.description);
    }
    if (fm.author !== undefined && (current.author ?? '').trim() === '') {
      setValue('author', fm.author);
    }
    if (fm.tags !== undefined && (current.tags ?? '').trim() === '') {
      setValue('tags', fm.tags.join(', '));
    }
    if (
      fm.date !== undefined
      ...
      setValue('date', fm.date.slice(0, 10));
    }
    toast.success(t('toast.parsedFileEn', { filename: file.name }));
  }, [setValue, getValues, t]);
      ...
      <div className="flex flex-col gap-1.5">
        <Label htmlFor={contentId} className={fieldLabelClass}>
          ...
          <span className="text-foreground-tertiary font-normal">
            ({t('fields.optional')} {t('fields.dropMdxHint')})
          </span>
        </Label>
        <div
          ref={dropRef}
