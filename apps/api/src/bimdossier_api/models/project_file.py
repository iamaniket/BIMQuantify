...
from bimdossier_api.models._mixins import (
    FileBackedMixin,
    SoftDeleteMixin,
    StoredFileMixin,
    TimestampMixin,
...
class ProjectFileRole(StrEnum):
    ...
    the actual content of a ``Document`` — they run the extraction pipeline and
    are what the 3D viewer renders. ``attachment`` rows are supporting files
    added at the project level. The CHECK constraints on the table tie
    ``document_id`` to this discriminator (a model source must belong to a
    Document; an attachment never claims one).
    """
...
class ProjectFile(TimestampMixin, SoftDeleteMixin, FileBackedMixin, StoredFileMixin, TenantBase):
    """A project-scoped stored file, playing one of two roles (see ``role``).

    ``model_source`` rows are versions of a ``Document`` (anchored by
    ``(document_id, version_number)``); ``attachment`` rows are supporting files
    ...
    # Page count for PDF document files (computed at extraction). NULL for IFC/DXF
    # files and rows extracted before this column existed. Lifted out of
    # Job.result JSONB so it is queryable and exposed in ProjectFileRead.
    ...
    # model_source rows leave this NULL and version by (document_id, version_number).
    ...
    document: Mapped[Document | None] = relationship(
        back_populates="files", foreign_keys=[document_id]
    )
    ...
    __table_args__ = (
        ...
        # A model source IS the content of a Document → must belong to one. An
        # attachment is never a document version → it never claims document_id.
        CheckConstraint(
            "role <> 'model_source' OR document_id IS NOT NULL",
            name="ck_project_files_model_source_has_document",
        ),
        CheckConstraint(
            "role <> 'attachment' OR document_id IS NULL",
            name="ck_project_files_attachment_no_document",
        ),
        # shared
        ...
        # model_source facet
        Index("ix_project_files_document_id", "document_id"),
        ...
        # Document versions: unique per document. Partial so attachment rows
        # (document_id NULL) are exempt.
        Index(
            "ux_project_files_document_version",
            "document_id",
            "version_number",
            unique=True,
        ...
        # Partial so model_source rows are exempt (they version by document above).
        Index(
