"""Pooled free-tier attachments — `public.free_attachments`.
...
link lives in `free_finding_attachment.FreeFindingAttachment`.

Pooled-in-`public`, isolation by owner-keyed RLS on `owner_user_id` plus
owner-OR-member visibility resolved through `free_project_id` (see
`_rls_sql.enable_free_attachment_rls_statements`). `owner_user_id` is the project
...
FREE_ATTACHMENT_CATEGORIES: tuple[str, ...] = tuple(c.value for c in AttachmentCategory)
...
class FreeAttachment(PooledOwnedMixin, TimestampMixin, MasterBase):
    ...
    __table_args__ = (
        ...
        Index(
            "uq_free_attachments_owner_idempotency_key",
            "owner_user_id",
            "idempotency_key",
            unique=True,
            postgresql_where=text("idempotency_key IS NOT NULL AND deleted_at IS NULL"),
        ),
        Index("ix_free_attachments_owner", "owner_user_id"),
        Index("ix_free_attachments_project", "free_project_id"),
        {"schema": "public"},
    )
