...
from bimdossier_api.notifications.pooled_service import CHANNEL_PREFIX_POOLED
...
class ConnectionManager:
    def __init__(self) -> None:
        ...
        # itself per-user (`notifications:pooled:user:<uid>`), so the user id is both
        # the fan-out key and the connection cap key.
        self._pooled_connections: dict[UUID, set[WebSocket]] = {}
    ...
    async def start(self, redis: Redis) -> None:
        ...
        await self._pubsub.psubscribe(f"{CHANNEL_PREFIX_POOLED}*")
    ...
    async def stop(self) -> None:
        ...
        self._pooled_connections.clear()
    ...
    async def connect_pooled(
        ...
        *,
        ...
        conns = self._pooled_connections.setdefault(user_id, set())
        if len(conns) >= max_per_user:
            if not conns:  # max_per_user <= 0: drop the empty set we just made
                self._pooled_connections.pop(user_id, None)
            return False
        conns.add(ws)
        try:
            await ws.accept(subprotocol=subprotocol)
    ...
    def disconnect_pooled(self, ws: WebSocket, user_id: UUID) -> None:
        conns = self._pooled_connections.get(user_id)
    ...
    async def _listener(self) -> None:
        ...
        while True:
            ...
            # Free-tier channel (`notifications:pooled:user:<uid>`) — per-user, so
