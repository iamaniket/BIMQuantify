...
async def _load_file_or_404(session: AsyncSession, project_id: UUID, file_id: UUID) -> ProjectFile:
    row = (
...
async def initiate_upload(
    ...
    storage_key = f"projects/{project.id}/{uuid4()}.ifc"
...
async def complete_upload(
    ...
    row = await _load_file_or_404(session, project.id, file_id)
    ...
    if result.rejection is None and result.schema is not None:
        ...
        try:
            ...
            logger.warning(
                "Extractor dispatch failed for %s: %s", row.storage_key, exc
            )
            await session.flush()
...
async def retry_extraction(
    ...
    try:
        ...
        row.extraction_error = f"DISPATCH_FAILED: {exc}"[:500]
        logger.warning(
            "Extractor re-dispatch failed for %s: %s", row.storage_key, exc
        )
        await session.flush()
...
async def get_viewer_bundle(
    ...
    if (
        ...
        or row.fragments_storage_key is None
    ):
        raise HTTPException(
    ...
    if row.metadata_storage_key is not None:
        metadata_url = await storage.presigned_get_url(
            row.metadata_storage_key, "metadata.json"
        )
