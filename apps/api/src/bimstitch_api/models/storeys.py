"""A building storey/level of a 3D Model.

Populated at extraction time from the IFC spatial tree (one row per
``IfcBuildingStorey``), via an idempotent upsert keyed by ``(model_id,
...
class Storey(TimestampMixin, SoftDeleteMixin, TenantBase):
    ...
    model: Mapped[Model] = relationship(back_populates="storeys")

    __table_args__ = (
        Index("ix_storeys_model_id", "model_id"),
        # Idempotency key for re-extraction upsert: one active storey per
        # (model, IFC GlobalId). Partial so rows without a GUID and soft-deleted
        # rows are exempt.
        Index(
            "uq_storeys_model_guid",
            "model_id",
            "ifc_guid",
            unique=True,
