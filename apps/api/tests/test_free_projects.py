...
async def test_free_project_thumbnail_upload(
    ...
    fetched = await client.get(f"/free/projects/{pid}", headers=_auth(token))
...
async def test_project_cap_is_configurable(
    ...
    try:
        ...
        over = await client.post(
            "/free/projects", json={"name": "second"}, headers=_auth(token)
        )
...
async def test_board_feed_and_overview(
    ...
    assert (
        await client.patch(
            f"/free/findings/{sid2}", json={"status": "in_progress"}, headers=_auth(token)
        )
    ).status_code == 200
    assert (
        await client.patch(
            f"/free/findings/{sid1}", json={"status": "resolved"}, headers=_auth(token)
        )
    ).status_code == 200

    # Board feed — FindingRead shape across the project's containers.
    board = await client.get(f"/free/projects/{pid}/findings", headers=_auth(token))
...
async def test_snag_status_accepts_paid_values(
    ...
    sid = (
        await client.post(
            f"/free/documents/{did}/findings", json={"title": "x"}, headers=_auth(token)
        )
    ).json()["id"]

    for st in ("draft", "open", "in_progress", "resolved", "verified"):
        r = await client.patch(f"/free/findings/{sid}", json={"status": st}, headers=_auth(token))
        assert r.status_code == 200, r.text
        assert r.json()["status"] == st

    # The legacy 'closed' value is no longer accepted.
    bad = await client.patch(
        f"/free/findings/{sid}", json={"status": "closed"}, headers=_auth(token)
    )
...
async def test_rls_isolation_free_projects(
    ...
    assert (
        await client.get(f"/free/projects/{pid}/documents", headers=_auth(token_b))
    ).json() == []
    ...
    assert (
        await client.post(
            f"/free/projects/{pid}/documents", json={"name": "hax"}, headers=_auth(token_b)
        )
    ).status_code == 404
